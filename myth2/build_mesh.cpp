...
int main(int argc, char* argv[]) {
    ...
    if (noBlob) {
        printf("--no-blob: dropping editor appendix and trailing_a; rebuilding entire post-action tail from cell data\n");
        printf("           media_coverage = cellH*16 bytes BE16 cellW*8 (runtime-default formula)\n");
        printf("           mesh_LOD_data  = generated from cell heights/flags (Loathing's algorithm)\n");
        printf("           fence connectors = generated from assets/terrain/fences.json\n");
    ...
    // In --no-blob mode, rebuild the entire post-action tail from generated bytes:
    //   [media_coverage_region][mesh_LOD_data][fence connectors]
    // No source blobs needed once cell records are available.
    if (noBlob) {
        std::vector<SourceCellRecord> tailCells = sourceCellRecords;
        if (tailCells.empty() && supportCellGrid.size() == (size_t)totalCells * 12) {
            tailCells.resize((size_t)totalCells);
            for (size_t i = 0; i < (size_t)totalCells; i++) {
                tailCells[i].raw.assign(supportCellGrid.begin() + (ptrdiff_t)(i * 12),
                                        supportCellGrid.begin() + (ptrdiff_t)((i + 1) * 12));
            }
        }

        if (tailCells.size() != (size_t)totalCells) {
            printf("--no-blob: WARNING no cell records loaded; keeping source mesh_LOD_data and media_coverage bytes\n");
        } else {
            // Media coverage region: cellH * 16 bytes filled with BE16 (cellW * 8).
            // Matches the runtime alloc/fill formula from Myth II decomp FUN_004b2580
            // (in 1.8.5 the engine doesn't actually regen this when size=0; we emit it
            // explicitly at the documented runtime default).
            std::vector<uint8_t> rebuiltMc((size_t)cellH * 16, 0);
            uint16_t mcVal = (uint16_t)((cellW * 8) & 0xFFFF);
            for (size_t i = 0; i + 1 < rebuiltMc.size(); i += 2) {
                rebuiltMc[i]     = (uint8_t)((mcVal >> 8) & 0xFF);
                rebuiltMc[i + 1] = (uint8_t)(mcVal & 0xFF);
            }

            // Mesh LOD data: generated from cells via Loathing's algorithm.
            std::vector<uint8_t> rebuiltLod = generateMeshLodFromCells(tailCells, cellW, cellH);

            // Connectors were generated earlier into rebuiltConnectorPayload. If the
            // early pass already spliced them into sourceTail, prefer the freshly
            // generated bytes here (we are about to discard sourceTail).
            std::vector<uint8_t>& rebuiltConn = rebuiltConnectorPayload;

            // Stitch the new tail.
            uint32_t newTailBase = sourceActionOffset + sourceActionSize;
            sourceTail.clear();
            sourceTail.insert(sourceTail.end(), rebuiltMc.begin(), rebuiltMc.end());
            sourceTail.insert(sourceTail.end(), rebuiltLod.begin(), rebuiltLod.end());
            sourceTail.insert(sourceTail.end(), rebuiltConn.begin(), rebuiltConn.end());

            // Update descriptor source variables so the header-write code emits the
            // right offsets and sizes for the rebuilt layout.
            sourceMediaCoverageOffset = newTailBase;
            sourceMediaCoverageSize   = (uint32_t)rebuiltMc.size();
            sourceMeshLodOffset       = newTailBase + (uint32_t)rebuiltMc.size();
            sourceMeshLodSize         = (uint32_t)rebuiltLod.size();
            sourceTrailingOffsetB     = newTailBase + (uint32_t)rebuiltMc.size() + (uint32_t)rebuiltLod.size();
            sourceTrailingSizeB       = (uint32_t)rebuiltConn.size();

            printf("--no-blob: regenerated media_coverage_region (%zu bytes, BE16 %u repeated)\n",
                   rebuiltMc.size(), (unsigned)mcVal);
    ...
    printf("  Post-action tail: %zu bytes%s\n", tailSz, noBlob ? " (regenerated)" : " (preserved from source)");
    printf("  Editor appendix: %zu bytes\n", sourceAppendix.size());
