...
2. **Decode `media_coverage_region` format.** Source on-disk MC starts with `06 00 06 00 06 00 ...` (uint16 BE = `cellW * 8`) which matches the runtime allocation/fill formula from the Ghidra decomp. Need to check whether the runtime regen path is reachable in gameplay vs editor only. If we can't trigger regen, decode the format from media-bearing maps and emit semantically.
