...
| --- | --- | --- | --- |
| media_coverage_region | 0xC0 / 0xC4 | **Required** but content-trivial | **Regenerated** by `build_mesh --no-blob` as `cellH * 16` bytes filled with BE16 `cellW * 8` — the runtime alloc/fill formula from the decomp. Empirically validated: a plugin with MC truncated to this 3,072-byte runtime default loads cleanly. The ~9 KB of editor-side records past the default region are not read at runtime. |
| mesh_LOD_data | 0xCC / 0xD0 | **Required** (despite Vengeance claim) | **Regenerated** by `build_mesh --no-blob` using Loathing's algorithm — see [mesh_lod_format.md](mesh_lod_format.md). 18432 B = (cellW/2) × (cellH/2) × 2 = 96×96 uint16 BE entries for a 192×192 cell mesh. |
...
1. ~~**Decode `mesh_LOD_data` format.**~~ **Done.** Algorithm extracted from Loathing 1.8.4 via Ghidra (FUN_004415d0 + FUN_00440730). Implementation in `build_mesh.cpp::generateMeshLodFromCells`. Validated empirically at 99.83% byte-match against Bungie's shipped le3e LOD; the ~0.17% diffs are single edge-bit values in row 0 / column 0 that appear to come from an algorithmic drift between Bungie's original generator and Loathing's recompute. See [mesh_lod_format.md](mesh_lod_format.md) for the format spec.
2. ~~**Decode `media_coverage_region` format.**~~ **Resolved.** On-disk MC starts with 1,536 entries of `0x0600 = cellW * 8` filling 3,072 bytes (= `cellH * 16`), which is exactly the runtime alloc/fill formula. The remaining ~9 KB of structured 5-uint16 records in the source were editor-only — a plugin with MC truncated to just the runtime-default 3,072 bytes loads cleanly with correct rendering. `build_mesh --no-blob` now generates MC directly from cellW and cellH.
3. **Decode the unknown 14 bytes of each connector record** (offsets 48-62). Vary wildly across records; possibly per-fence rendering hints, possibly uninitialized editor heap. Empirical test loaded le3e cleanly with these zeroed, so they may be ignorable.
4. **Investigate `drop_model_for_current_frame` crash** for completeness. Use Ghidra to read the offset 0x7E instruction in model_animations.c:575 to see what specific field is being dereferenced. Already empirically resolved (preserving MC + correct LOD makes the load clean), but the deterministic answer would be useful documentation.
